# https://github.com/sinanejadebrahim/jira-crack/blob/main/README.md

make up

docker exec -it jira_jira_1 /bin/sh

    cd /opt/jira/atlassian-jira/WEB-INF/lib/
    или
    cd /opt/atlassian/jira/atlassian-jira/WEB-INF/lib/

    rm -f ./atlassian-extras-3.4.6.jar
    
    wget https://omwtfyb.ir/atlassian-extras-3.4.6.jar

    cd /opt/jira/bin/
    или
    /opt/atlassian/jira/bin/
    
    ./start-jira.sh
    ./stop-jira.sh
    ./start-jira.sh

Перезагрузить контейнер

http://127.0.0.1:8085/

вставить: X02o0

если не принимает ключ, то надо перезагрузить (restart) контейнер (jira_jira_1) с джирой